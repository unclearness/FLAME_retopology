...
class ObjMesh:
    ...
    def recomputeNormals(self):
        v0 = self.verts[self.indices[..., 0]]
        v1 = self.verts[self.indices[..., 1]]
        v2 = self.verts[self.indices[..., 2]]
        v10 = v1 - v0
        v20 = v2 - v0
        v10 /= np.linalg.norm(v10, axis=1, keepdims=True)
        v20 /= np.linalg.norm(v20, axis=1, keepdims=True)
        self.face_normals = np.cross(v10, v20, axis=1)
        self.face_normals /= np.linalg.norm(self.face_normals,
                                            axis=1, keepdims=True)
        # TODO: batch
        count = np.zeros(self.verts.shape[0], dtype=int)
        tmp_sum = np.zeros_like(self.verts)
        for fid in range(len(self.indices)):
            fn = self.face_normals[fid]
            for i in range(3):
                vid = self.indices[fid][i]
                count[vid] += 1
                tmp_sum[vid] += fn
        invalid_mask = count < 0
        count[invalid_mask] = 1
        normalized = tmp_sum / count[..., None]
        normalized /= np.linalg.norm(normalized, axis=1, keepdims=True)
        self.normals = normalized
